...
"""
 ...
 These are the main steps:
    ...
    * Load mesh in pyACP and build layup
    * Export CompositeDefinitions and Material data from pyACP.
    * Import Layup and material in pymechanical (Imported Plies)
...
# Import the geometry
# A agdb (Design modeler) file is used, because this allows to specify Named Selections
...
# to the path and then just pass the filename. This does not work for the embedded case
# (and vice-versa).
import clr
...
# Export hdf5 file from Mechanical
...
# Create Structural Analysis. This is done here
# to know the solver_files_directory
analysis = Model.AddStaticStructuralAnalysis()
solver_files_directory = Model.Analyses[0].Children[0].SolverFilesDirectory

# Create setup folder for CompositeDefinitions file.
# It is important to save the CompositeDefinitions file in a location where a
...
# Identify result and material file
rst_file = [
    filename
